...
const AgentCard = ({ id, name, email, avatar, noOfProperties }: AgentCardProp) => {
   ...
   const generateLink = () => {
      return (currentUser.email === email) ? ('/my-profile') : (`show/${id}`)
   }
   ...
   return (
      ...
      >
         ...
         <Stack direction='column' justifyContent='space-between' gap={{ xs: 4, sm: 2 }}>
            ...
            <Stack direction='row' flexWrap='wrap' justifyContent='space-between' alignItems='center' gap={2}>
               ...
               <InfoBar icon={<Phone sx={{ color: '#808191' }} />} name='9898987272' />
